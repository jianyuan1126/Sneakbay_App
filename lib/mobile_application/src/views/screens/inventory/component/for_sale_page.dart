...
class ForSalePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (user == null) {
      return Scaffold(
        ...
        body: const Center(
            child: Text('You need to be logged in to view your listings.')),
      );
    }
                ...
                builder: (context, shoeSnapshot) {
                  ...
                  return Dismissible(
                    ...
                    onDismissed: (direction) async {
                      await _deactivateListing(
                          user.uid, document.id, shoeModel.id);
                      if (context.mounted) {
  ...
  Future<void> _deactivateListing(
      String userId, String documentId, String shoeId) async {
    ...
    final listingData = (await userDoc.get()).data();
    if (listingData != null) {
      listingData['status'] = 'draft';
      listingData.remove('listingId');

      await userDraftsDoc.set(listingData);
      await userDoc.delete();
      await allListingsDoc.delete();
    }
  }
}
              ...
              children: [
                Row(
                  children: [
                    ...
                    GestureDetector(
                      onTap: () async {
                        final newPrice = await Navigator.push(
                          context,
                          MaterialPageRoute(
                        ...
                        if (newPrice != null && context.mounted) {
                          double earnings = _calculateEarnings(newPrice);
